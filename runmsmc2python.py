...
for filename in os.listdir(input_directory):
    if filename.endswith(".multihetsep.txt"):
            call (["./msmc2_linux64bit","-I0,1","-p1*2+15*1+1*2","-o",outdir+'2hap'+'%02d_%s' %(hap_counter,filename),filename])
            hap_counter+= 1
            call (["./msmc2_linux64bit","-I2,3","-p1*2+15*1+1*2","-o",outdir+'2hap'+'%02d_%s' %(hap_counter,filename),filename])
            hap_counter+= 1
            call (["./msmc2_linux64bit","-I4,5","-p1*2+15*1+1*2","-o",outdir+'2hap'+'%02d_%s' %(hap_counter,filename),filename])
            hap_counter+= 1
