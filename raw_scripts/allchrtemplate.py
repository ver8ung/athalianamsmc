...
for file in os.listdir(input_directory): # retrieve filenames in input_directory for output prefix
    if fnmatch.fnmatch(file, "*madeira*.txt"): # call MSMC with desired parameters on specified input files
...
for file in os.listdir(input_directory): # retrieve filenames in input_directory for output prefix       
     if fnmatch.fnmatch(file, "*madeira*_reduced*.txt"): # call MSMC with desired parameters on specified input files
      while hap_counter < 27:
              outputprefix = file[:31]   # use only desired positions of input filenames as output prefix
              call (["./msmc_1.1.0_linux64bit","-I","%s,%s,%s,%s" %(index_counter,index_counter_1,index_counter_2,index_counter_3),"-R","-r",str(r_average),"-o",outdir+'4hap_fixeddef_nocent'+'%02d_%s' %(hap_counter,outputprefix),"%s" %(all_chr_input_madeira_reduced[0]),"%s" %(all_chr_input_madeira_reduced[1]),"%s" %(all_chr_input_madeira_reduced[2]),"%s" %(all_chr_input_madeira_reduced[3]),"%s" %(all_chr_input_madeira_reduced[4])])
              hap_counter += 1
              index_counter += 4
              index_counter_1 += 4
              index_counter_2 += 4
